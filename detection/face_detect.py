...
class MTCNN:
    def __init__(self, model_path):
        ...
        # 获取P模型
        P_path = os.path.join(model_path, 'PNet.pth')
        P_path = P_path.replace('\\', '/')
        self.pnet = torch.jit.load(P_path)
        self.pnet.to(self.device)
        self.softmax_p = torch.nn.Softmax(dim=0)
        self.pnet.eval()

        # 获取R模型
        R_path = os.path.join(model_path, 'RNet.pth')
        R_path = R_path.replace('\\', '/')
        self.rnet = torch.jit.load(R_path)
        self.rnet.to(self.device)
        self.softmax_r = torch.nn.Softmax(dim=-1)
        self.rnet.eval()

        # 获取O模型
        O_path = os.path.join(model_path, 'ONet.pth')
        O_path = O_path.replace('\\', '/')
        self.onet = torch.jit.load(O_path)
